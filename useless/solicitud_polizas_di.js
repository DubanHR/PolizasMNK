...
const { WEBHOOK_VERIFY_TOKEN, GRAPH_API_TOKEN, FLOW_ID, FLOW_ID_2, URL_SERVICE, Authorization, PORT } = process.env;
...
app.post("/webhook", async (req, res) => {
  ...
  if (message?.type === "text") {
    ...
    if(message.interactive.button_reply.id === "SOLICITAR_ACUERDO"){  
      ...
      mensaje = "Presiono el boton consultar *POLIZA*";
      mensajeTexto(mensaje);
      
    }else if(message.interactive.button_reply.id === "PREGUNTAS_FRECUENTES"){ 
      
      mensaje = "www.inteligencia-movil.com/";
      mensajeTexto(mensaje);
              ...
              "parameters": [
                {
                  "type": "image",
                  "image": {
                    "link": "http://invesvial.com/IM/datosInteligentes/app/logo-icondatosinteligentes.png"
...
function generarToken() {
  ...
  const data = {
      'data':{
          ...
          "clave": "b299567105832ab6sssaJh+ChD1Mclh7eie42Q=="
      },
      'platform': "PolizaWhatsApp"
  }; 
  ...
  fetch(url, {
      ...
      if(data.statusCode === 1){
        ...
        console.error('❌ Error genenrardo TOKENT');
      }else{
        console.error('❌ Error genenrardo TOKENT');
      }
  }).catch((error) => {
      console.error('❌ Error genenrardo TOKENT:', error); 
  });   
}
